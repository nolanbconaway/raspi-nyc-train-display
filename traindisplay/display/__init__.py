...
def display_train_times(
    ...
    for num, dt in enumerate(stops_dt_str):
        text = pygame.font.SysFont(text_font_path, 72).render(dt, True, text_color)
    ...
    pygame.draw.circle(screen, text_color, (int(width / 3), int(height / 2)), 100)

    # add train logo letter
    text = pygame.font.SysFont(text_font_path, 180).render(route_id, True, (0, 0, 0))
    screen.blit(
        text, (width / 3 - text.get_width() // 2, height / 2 - text.get_height() // 2)
    )
